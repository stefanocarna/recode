		...
		const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*maskp & DM_G_LLC_FLUSH_SHIFT)
		enable_mitigations_global(DM_G_LLC_FLUSH);
	else
		disable_mitigations_global(DM_G_LLC_FLUSH);


	if (*maskp & DM_G_TE_MITIGATE_SHIFT)
		enable_mitigations_global(DM_G_TE_MITIGATE);
	else
		disable_mitigations_global(DM_G_TE_MITIGATE);


	if (*maskp & DM_G_CPU_EXILE_SHIFT)
		enable_mitigations_global(DM_G_CPU_EXILE);
	else
		disable_mitigations_global(DM_G_CPU_EXILE);


	if (*maskp & DM_G_VERBOSE_SHIFT)
		enable_mitigations_global(DM_G_VERBOSE);
	else
		disable_mitigations_global(DM_G_VERBOSE);
...
int recode_register_proc_mitigations(void)
{
