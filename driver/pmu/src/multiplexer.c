...
uint pmc_collect_partial_values(uint cpu, uint gp_cnt, uint index)
{
	uint pmc, hw_cnt;
	pmc_ctr value;
	...
	if (hw_cnt <= gbl_nr_pmc_general) {
		for_each_active_fixed_pmc(ctrl, pmc) {
			...
			pr_debug("** [PMC-SLOW] FIXED %u [%llx -> %llx = %llx]\n", pmc,
...
static bool pmc_multiplexing_access(uint cpu)
{
	uint req_hw_events;
	uint index = per_cpu(pcpu_pmus_metadata.hw_events_index, cpu);
	...
	if (req_hw_events == 0) {
		pmc_ctr tsc;
		uint pmc;
		uint scale = per_cpu(pcpu_pmus_metadata.pmi_partial_cnt, cpu);
		struct pmcs_collection *pmcs_collection =
			per_cpu(pcpu_pmus_metadata.pmcs_collection, cpu);
	...
	fast_setup_general_pmc_local(hw_events->cfgs, index, req_hw_events);
...
static bool pmc_fast_access(uint cpu)
{
	uint pmc;
	pmc_ctr tsc;
	pmc_ctr value;
...
bool pmc_access_on_pmi(uint cpu)
{
	uint multiplexing = per_cpu(pcpu_pmus_metadata.multiplexing, cpu);
